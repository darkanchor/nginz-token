...
extern var ngx_http_llm_metrics_module: ngx_module_t;
extern var ngx_http_llm_fallback_module: ngx_module_t;
extern var ngx_http_llm_ratelimit_module: ngx_module_t;
extern var ngx_http_llm_cost_module: ngx_module_t;
extern var ngx_http_llm_cache_module: ngx_module_t;
...
export const ngx_modules = [_][*c]ngx_module_t{
    ...
    // nginz-token filter modules (between userid and headers)
    // llm_ratelimit must be registered before llm_proxy so that nginx's reversed
    // phase-handler ordering puts ratelimit AFTER proxy in the flat execution array.
    // Ratelimit's ACCESS handler then runs in the second pass (after proxy has parsed
    // the body and set $llm_model), so model tier overrides work correctly.
    &ngx_http_llm_ratelimit_module,
    &ngx_http_llm_proxy_module,
    &ngx_http_llm_auth_module,
    &ngx_http_llm_metrics_module,
    &ngx_http_llm_fallback_module,
    &ngx_http_llm_cost_module,
    &ngx_http_llm_cache_module,
    &ngx_http_llm_security_module,
